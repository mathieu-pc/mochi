...
def refineGrid(cells, positions, radii, threshold, cellsOver = None, cellsMasks = None):
	"""
	...
	for n in range(cellsNumber):
		...
		if minRadius * threshold < size:
			...
			newCellsOver.append(True)
			newCellsMasks.append([True])
	return np.array(newCells) if len(newCells) == len(cells) else refineGrid(newCells, positions, radii, threshold, newCellsOver, newCellsMasks)
...
def makeAdaptiveCube(particles, xRange, interpolant, kernel, channelSize, *, initialGridSize = 2, threshold = 0.5, trigger = 1, minimumElement = 1 * units.kpc):
	"""
	...
	finalCells = refineGrid(initialCells, positions, radii, threshold)
